...
import { Tooltip } from '../Button/node_modules/@infraUI/display';
...
export const CalendarPageDate: React.FC<CalendarPageDateProps> = ({
  ...
  const getStartDate = (startDate: Date, statusClassName: string) => {
    return (
      startDate && (
        <div className="ee-calendar-page-date-wrapper-start">{renderCalendarPage(startDate, statusClassName)}</div>
      )
    );
  };
  ...
  const getEndDate = (endDate: Date, statusClassName: string) => {
    return (
      endDate && (
        <div className="ee-calendar-page-date-wrapper-end">{renderCalendarPage(endDate, statusClassName, 'end')}</div>
      )
    );
  };
